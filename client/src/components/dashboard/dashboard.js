...
function dashboard(props) {
  ...
  const getRiotUsername = () => {
    ...
    fetch(url)
      ...
      .then(() => {
        ...
        setErrorStyle("pt-2 text-success")
      })
      .catch((err) => {
        console.error(err);
        setAddingAccount(false);
  ...
  const updateUserList = (data) => {
    ...
    .then(response => {
      ...
      renderErrorMsg("Valorant account added!")}, 
  ...
  return (
    showStatsPage ? (
      ...
      <div className="d-flex flex-column bg-dark border border-dark rounded p-5 align-items-center w-100">

        <h1 className="h1">Dashboard</h1>

        <div className="m-3 p-3">
          <h1 className="h3">Welcome, {userData ? (userData.username) : ("NULL USERDATA")}!</h1>
        </div>
